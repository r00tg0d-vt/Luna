...
st.write(
    f"Scanning the next **{SCAN_DAYS} days** using **{TIMEZONE}** timezone."
)
...
def scan_availability(progress_bar, status_text):
    ...
    for block_number, day_offset in enumerate(
        ...
        status_text.write(
            f"Scanning block **{block_number}/{total_blocks}** · {start_date} → {end_date}"
        )
        ...
        try:
            ...
            data = response.json()
            days = data.get("calendly_events", {}).get("days", [])
